...
class StockPriceDataset(Dataset):
    ...
    @staticmethod
    def ds_state(ds_df):
        std = ds_df.std(numeric_only=True)
        mean = ds_df.mean(numeric_only=True)
    ...
    def __getitem__(self, idx):
        ...
        return torch.from_numpy(self._data[idx, ...]).float(), torch.from_numpy(np.expand_dims(self._true[idx, ...], axis=-1)).float()
